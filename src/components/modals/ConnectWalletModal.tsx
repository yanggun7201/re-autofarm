import React, { useCallback } from "react";
import { css } from "@emotion/react";
import { Theme } from "../../theme";
import bscWalletJPG from "../../images/wallets/bscwallet.jpg";
import { useConnectWalletContext } from "../../core/contexts/ConnectWalletContext";
import PureModal from 'react-pure-modal';
import useSetState from "../../core/hooks/useSetState";

type Props = {
    className?: string,
};

type DEFAULT_STATE = {
    walletName?: string | null,
};

const DEFAULT_APP_STATE = {
    walletName: null,
};

const ConnectWalletModal: React.FC<Props> = ({ className = "" }) => {
    const [state] = useSetState<DEFAULT_STATE>(DEFAULT_APP_STATE);

    const { isOpen, toggleConnectWalletModal, connectWallet } = useConnectWalletContext();

    const onRequestClose = useCallback(() => {
        toggleConnectWalletModal(false);
    }, [toggleConnectWalletModal]);

    const onWalletClicked = useCallback((walletName: string) => {
        // setState({ walletName });
        connectWallet();
        toggleConnectWalletModal(false);
    }, [/** setState, */ connectWallet, toggleConnectWalletModal]);

    return (
        <PureModal
            isOpen={isOpen}
            onClose={onRequestClose}
            css={style}
            className={className}
        >
            <div css={style}>
                <div css={walletBoxStyle(state.walletName === "MetaMask")}>
                    <div css={walletInnerBoxStyle} onClick={() => onWalletClicked("MetaMask")}>
                        <div css={iconContainerStyle}>
                            <img
                                src="data:image/svg+xml;base64,"
                                alt="MetaMask"
                            />
                        </div>
                        <div css={titleStyle}>MetaMask</div>
                        <div css={descriptionStyle}>Connect to your MetaMask Wallet
                        </div>
                    </div>
                </div>
                <div css={walletBoxStyle(state.walletName === "WalletConnect")}>
                    <div css={walletInnerBoxStyle} onClick={() => onWalletClicked("WalletConnect")}>
                        <div css={iconContainerStyle}>
                            <img
                                src="data:image/svg+xml;base64,"
                                alt="WalletConnect"
                            />
                        </div>
                        <div css={titleStyle}>WalletConnect</div>
                        <div css={descriptionStyle}>Scan with WalletConnect to
                            connect
                        </div>
                    </div>
                </div>
                <div css={walletBoxStyle(state.walletName === "BinanceChainWallet")}>
                    <div css={walletInnerBoxStyle} onClick={() => onWalletClicked("BinanceChainWallet")}>
                        <div css={iconContainerStyle}>
                            <img src={bscWalletJPG} alt="Binance Chain Wallet" />
                        </div>
                        <div css={titleStyle}>Binance Chain Wallet</div>
                        <div css={descriptionStyle}>Binance Smart Chain Wallet
                        </div>
                    </div>
                </div>
            </div>
        </PureModal>
    );
}

const style = (theme: Theme) => css`
    position: relative;
    border-radius: 12px;
    margin: 0;
    padding: 0;
    opacity: 1;
    visibility: visible;
    pointer-events: auto;
    display: grid;
    grid-template-columns: repeat(auto-fit, minmax(320px, 1fr));
    overflow: auto;

    max-width: 800px;
    min-width: fit-content;
    max-height: 100%;

    background-color: ${theme.colours.connectWallet.background};

    ${theme.breakpoints.up("md")} {
        min-width: 800px !important;
    }

    ${theme.breakpoints.down("sm")} {
        min-width: calc(90vw) !important;
        grid-template-columns: 1fr;
    }

    ${theme.breakpoints.down("xs")} {
        min-width: 100%;
        max-width: calc(100% - 50px) !important;
        grid-template-columns: 1fr;
    }

    img {
        width: 100%;
        height: 100%;
    }

    .close {
        display: none;
    }

    .panel {
        .panel-body {
            overflow: hidden;
            border-radius: 12px;
            padding: 0;
            margin: 0;
        }
    }
`;

const iconContainerStyle = (theme: Theme) => css`
    width: 45px;
    height: 45px;
    display: flex;
    border-radius: 50%;
    overflow: visible;
    box-shadow: none;
    -webkit-box-pack: center;
    justify-content: center;
    -webkit-box-align: center;
    align-items: center;
`;

const walletBoxStyle = (isActive: boolean) => (theme: Theme) => css`
    width: 100%;
    padding: 8px;
    display: flex;
    -webkit-box-pack: center;
    justify-content: center;
    -webkit-box-align: center;
    align-items: center;
    flex-direction: column;
    cursor: pointer;
    border-radius: 0px;
    border: 1px solid rgba(195, 195, 195, 0.14);
    background-color: ${theme.colours.connectWallet.background};
    transition: background-color ${theme.transitions.transitions};

    ${isActive && css`
        background-color: ${theme.colours.connectWallet.hover};
    `};
`;

const walletInnerBoxStyle = (theme: Theme) => css`
    width: 100%;
    display: flex;
    flex-direction: column;
    -webkit-box-pack: center;
    justify-content: center;
    -webkit-box-align: center;
    align-items: center;
    border-radius: 12px;
    padding: 24px 16px;
    text-align: center;
    background-color: ${theme.colours.connectWallet.background};
    transition: background-color ${theme.transitions.transitionSlower};

    :hover {
        background-color: ${theme.colours.connectWallet.hover};
    }
`;

const titleStyle = (theme: Theme) => css`
    width: 100%;
    font-size: 24px;
    font-weight: bold;
    margin-top: 0.5em;
    color: ${theme.colours.connectWallet.title};

    ${theme.breakpoints.down("sm")} {
        font-size: 5vw;
    }
`;

const descriptionStyle = (theme: Theme) => css`
    width: 100%;
    font-size: 18px;
    margin: 0.333em 0;
    color: ${theme.colours.connectWallet.description};

    ${theme.breakpoints.down("sm")} {
        font-size: 4vw;
    }
`;

export default ConnectWalletModal;

